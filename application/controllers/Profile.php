...
";
	...
	public function viewprofile($uuid = '')
	{
		...
		if($Profile){
			...
			if(isset($logged_profile_uuid) ){
				...
				$sharedId = '';
				$status = $this->Common_model->update_client_views($uuid,'view',$sharedId,0);
				if((float) $status > 0 ){
					$data['is_free'] = 0;
					$this->load->view('templates/header');
	...
	public function view_hidden_details($uuid){
		$logged_profile_uuid = $this->session->userdata('uuid');
		if($this->input->post('sharedId') ){
		...
		if(isset($logged_profile_uuid) ){
			$status = $this->Common_model->update_client_views($uuid,$type,$sharedId,1);
			if((float) $status < 1 ){
				$data = array(
					"status"=>0
				);
		...
		if($Shared){
			if((float) $Shared->shared_profile_status > 0 ){
				...
				if(isset($logged_profile_uuid) ){
					...
					$status = $this->Common_model->update_client_views($uuid,'shared',$share_uuid,0);
					if((float) $status > 0 ){
