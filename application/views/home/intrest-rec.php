...
<script>
	 ...
	 $.ajax({
             url: "<?php echo base_url(); ?>Profile/ProfileIntrest/received",
			 type:"POST",
			 data:{},
             async:false,
             success: function(result) {
				var row_serach_profile = '<div class="col-md-12"><h4>Interest Received</h4> </div>';
				$.each(JSON.parse(result), function(i, itemsingle) {
							var clsbtn = '';
					...
					row_serach_profile = row_serach_profile + '<div id="div_profile" class="col-md-6"><div class="row inner_matchprofile"><div class="col-md-4"> <a href = "<?php echo base_url(); ?>Profile/ViewProfile/'+itemsingle.profile_uuid+'"><img src="<?php echo base_url();?>uploads/profile/'+itemsingle.pro_image+'"></a> </div>';
					row_serach_profile = row_serach_profile + ' <div class="col-md-8 div_profile"> <h3>'+itemsingle.profile_name+'</h3> <p><p>'+itemsingle.occupation+'</p><p>'+itemsingle.high_education_name+'</p>';
					...
					if(parseFloat(itemsingle.reqstatus) < 1  ){ 
						...
						<?php $profile_uuid = $this->session->userdata('uuid');	 if(isset($profile_uuid) ){?>
							if(itemsingle.p_uuid === itemsingle.client_request_from){
								...
								if(itemsingle.client_request_status === 'pending'){
									row_serach_profile = row_serach_profile  + '<a href="#" uuid = "'+itemsingle.profile_uuid+'"  status="accepted"  class="intrestpopup Acc">Accept</a>&nbsp;';
									row_serach_profile = row_serach_profile  + '<a href="#" uuid = "'+itemsingle.profile_uuid+'"  status="rejected"  class="intrestpopup Rej">Reject</a>&nbsp;';
									row_serach_profile = row_serach_profile  + '<a href="#" uuid = "'+itemsingle.profile_uuid+'"  status="pending"  class="intrestpopup Pen">Keep Pending</a>';
								}else{
									row_serach_profile = row_serach_profile  +'<a href="#" uuid = "'+itemsingle.profile_uuid+'" style = "bg-color:green !important" class="'+clsbtn+'" >'+itemsingle.client_request_status+'</a>';
								}
							 }
