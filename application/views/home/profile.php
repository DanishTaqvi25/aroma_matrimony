  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-10">
		...
		<!--	<?php //if($is_free > 0  ){?>-->
				<h4>Contact Information	&nbsp;&nbsp; <button id= "view_details" type = "button" style = "border:0px;color:white" > <i style = "color:black" class="fa fa-eye" aria-hidden="true"></i></button>
			<span id = "updateaccount"></span></h4>
				<p id ="email_id">Email Id: ************ HIDDEN INFORMATION *******************</p>
				<p id ="mobile_no">Mobile Number: ********* HIDDEN INFORMATION ***********</p>
				<p id = "Whatsapp">Whatsapp Number: ************ HIDDEN INFORMATION **********</p>
				<p id ="Address">Address: ************ HIDDEN INFORMATION *******************</p>
				<p id = "Address2">Address 2: ************ HIDDEN INFORMATION *******************</p>
		<!--	<?php //}else {?>-->
				<!--<h4><span></span></h4>
				<p id ="email_id"> </p>
				<p id ="mobile_no"></p>
            ...
            <table>
              <tbody>
                <tr>
                  ...
                  <td id ="td_partner_anual_income">Monthly Income: 600000 & Above</td>
                </tr>
                <tr>
...
<script>
  ...
  $(document).ready(function() {
	$('#view_details').click(function(){
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>Profile/view_hidden_details/"+uuid,
			success: function(result) {
			var res = JSON.parse(result);
			var status = res.status;
			if(status > 0 ){
				$('#view_details').attr('disabled',true)
				$('#email_id').html('Email Id: '+res.profile.email);
				$('#mobile_no').html('Mobile Number: '+res.profile.mobile);
				$('#Whatsapp').html('Whatsapp Number: '+res.profile.whatsapp);
				$('#Address').html('Address :'+res.profile.address_1);
				$('#Address2').html('Address 2 :'+res.profile.address_2);
			}else{
				$('#updateaccount').attr('style','color:red;');
				$('#updateaccount').html('To Unlock For Accepted Profiles, Please Upgrade Your Account');
			}
        }
		});
	});
    ...
    $.ajax({
        ...
        $('#view_profile_id').html('Profile Id : AMA'+itemsingle.profile_register_id);
				var dobtext = '';
				if(itemsingle.dateofbirth !== ''){
					dobtext = ' ('+itemsingle.dateofbirth+') ';

				}
				$('#view_dob').html('Date of Birth: '+itemsingle.dateofbirth);
				$('#view_amaid').html('Profile ID: AMA'+itemsingle.profile_register_id);
				$('#view_profile_education').html(itemsingle.high_education_name);
				$('#view_age_mothertongue').html('Age: '+itemsingle.age+ dobtext +' | Mother Tongue: '+itemsingle.mothertongue);
				...
				$('#view_age_mothertongue1').html('Age:  '+itemsingle.age+dobtext+' <span>Mother Tongue: '+itemsingle.mothertongue+'</span>');
			...
			//	$('#td_partner_anual_income').html('Monthly Income :');
	...
	$.ajax({
        ...
        success: function(result) {
            $.each(JSON.parse(result), function(i, itemsingle) {
				$('#td_partner_anual_income').html('Monthly Income :'+itemsingle.p_income);
