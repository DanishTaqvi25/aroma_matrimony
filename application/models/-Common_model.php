...

{
	...
	function profile_partner_details($uuid){
	 ...
	 function profile_details($uuid){
        $this->db->select('IFNULL(occupation_name,"") as occupation_name,he.high_education_name,division.division_name,sub.sub_caste_name,religion.religion_name as religion_name,pstate.state_name as present_state_name,IFNULL(pcountry.country_name,"" ) as present_country_name,d.religion_name,c.mother_name as mothertongue,b.caste_name,a.*');
	...
	public function update_client_views($uuid,$type = 'view',$sharedId){
		$logged_profile_uuid = $this->session->userdata('uuid');
		$package = $this->getActivePackage($logged_profile_uuid);
	...
	function Shared_by_details($uuid){
		$qry ="SELECT b.profile_name,b.age,country_name,state_name,c.pro_image FROM `shared_profile` as a left JOIN client as b on a.`shared_profile_shared_by` = b.profile_uuid left JOIN client_image as c on c.profile_uuid = a.shared_profile_shared_by LEFT JOIN sm_country as country on country.country_uuid = b.country LEFT JOIN sm_state as state on state.state_uuid = b.state WHERE a.shared_profile_uuid = '".$uuid."' GROUP by b.profile_uuid";
		$query = $this->db->query($qry)->result_array();
		return $query;
	}
